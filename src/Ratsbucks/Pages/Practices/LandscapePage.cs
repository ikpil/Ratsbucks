...
public class LandscapePageState
{
    public PageType Type { get; set; }
}

public class LandscapePage : Component<LandscapePageState>
{
    ...
    private VisualNode RenderLandscape()
    {
        return new Grid("*", "64, *")
        {
            ...
            new FadeIn()
                {
                    new Label(State.Type + " Landscape")
                        .FontSize(24)
                        .TextColor(Colors.White)
                }
                .GridColumn(1)
...
abstract class TabComponent<T> : Component<T> where T : TabState, new()
{
    ...
    protected Image RenderTabItem(PageType pageType)
    {
        ...
        switch (pageType)
        {
            case Practices.PageType.Map:
                imageSource = "tab_map.png";
                break;
            case Practices.PageType.Favorites:
                imageSource = "tab_favorites.png";
                break;
            case Practices.PageType.Settings:
...
class HorizontalTab : TabComponent<HorizontalTabState>
{
    ...
    private void UpdateLinePositions()
    {
        SetState(s =>
        {
            if (s.Width != null)
            {
                ...
                s.TargetRight = (float)(((int)_pageType * s.Width / 4) + (s.Width.Value / 4.0 - tabWidth) / 2.0f +
                                        tabWidth);
            }
        });
    }
    ...
    public override VisualNode Render()
    {
        return new Grid(grid => _grid = grid)
            {
                RenderTabItem(Practices.PageType.Home),
                RenderTabItem(Practices.PageType.Favorites).GridColumn(1),
                RenderTabItem(Practices.PageType.Map).GridColumn(2),
                RenderTabItem(Practices.PageType.Settings).GridColumn(3),
...
class VerticalTab : TabComponent<VarticalTabState>
{
    ...
    private void UpdateLinePositions()
    {
        SetState(s =>
        {
            if (s.Height != null)
            {
                ...
                s.TargetBottom = (float)(((int)_pageType * s.Height / 4) + (s.Height.Value / 4.0 - tabHeight) / 2.0f +
                                         tabHeight);
            }
        });
    }
    ...
    public override VisualNode Render()
    {
        return new Grid(grid => _grid = grid)
            {
                RenderTabItem(Practices.PageType.Home),
                RenderTabItem(Practices.PageType.Favorites).GridRow(1),
                RenderTabItem(Practices.PageType.Map).GridRow(2),
                RenderTabItem(Practices.PageType.Settings).GridRow(3),
